...
public class BaseController extends SuperController implements HandlerInterceptor {
	...
	protected <T> Page<T> getPage(int size) {
		...
		if (request.getParameter("_sortName") != null) {
			_sortName = request.getParameter("_sortName").toString();
		}
		if (request.getParameter("_sortOrder") != null) {
			flag = "asc".equals(request.getParameter("_sortOrder"))?true :false;
		}
		Page<T> page = new Page<T>(_index, _size,_sortName);
		page.setAsc(flag);
		return page;
	}
